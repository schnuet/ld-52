...
onready var background_tween = $BackgroundTween;
...
func _thread_done(resource):
	...
	progress.hide();
	background.hide();
	$Progress/AnimationPlayer.stop(true);
	hide();
...
func load_scene(path):
	...
	background_tween.interpolate_property(background, "modulate", Color.transparent, Color.white, 2);
	background_tween.start();
	$Progress/AnimationPlayer.play("New Anim");
